...
From these results it is evident that the combination of batch normalisation and residual connections was a great solution to the vanishing/exploding gradient problem. 

Other possible fixes for this problem are: training shallower networks and gradually add more layers, using modified optimisation algorithms (such as RMSProp and gradient clipping), using modified hidden unit transfer functions (such as LSTM), or using a weight filler (such as Xavier initialization).
