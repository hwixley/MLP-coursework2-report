...
Training deep neural networks is complicated due to a problem called "internal covariate shift". This problem occurs when the distribution of each layer's inputs varies during training as the parameters of the preceding layers change. This makes it infamously difficult to train models with saturating non-linearities as it necessitates lower learning rates and careful parameter setup. In order to address this problem we normalize the layer inputs by a process called batch normalisation. Batch normalization is a technique for training very deep neural networks that standardizes the inputs to a layer for each mini-batch. This technique allows us to use much higher learning rates, be less careful about weight initialisation, and can even act as a regularizer (in some cases it can eliminate the need for smoothing techniques such as Dropout).

So how do we train a network with batch normalization?
Batch normalization is 

\underline{Batch Normalizing Transform applied to activation $a$}\\
\underline{over a mini-batch $\mathcal{B}$:}\\
Consider a mini-batch $\mathcal{B}$ of size m. Since the normalization is applied to each activation independently, let us focus on a particular activation $a^{k}$ and omit $k$ for clarity. We have m values of this activation in the mini-batch
\[\mathcal{B} = \{a_1...a_m\}\]
\begin{center}
where $a_i = w_i x$    
\end{center}
...
For a given epoch, this is completed for all the activations in all the layers of the network, once this has all been computed the $\gamma$ and $\beta$ parameters are then trained through gradient descent in conjunction with all the other network parameters.


How do we implement batch normalisation at test time?
Batch normalisation is simply a technique for helping us to optimise the weights of a deep neural network and thus does not affect the testing process at all since these weights have already been derived.
