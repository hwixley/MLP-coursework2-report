...
Now how do we perform batch normalisation in training?



Batch Normalizing Transform applied to activation $a$ over a mini-batch $\mathcal{B}$:

Let $\mathcal{B} = \{a_1...a_m\}$, $|\mathcal{B}| = M$, and $a_i = w_i x$
...
\begin{enumerate}
    ...
    where $\epsilon$ is a very small constant value added to the variance to prevent us from dividing by 0
